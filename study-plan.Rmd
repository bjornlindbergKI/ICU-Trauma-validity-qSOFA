...
Sample characteristics are shown in table \@ref(tab:table1). <!-- It's common to also summarize the data in the text, and state the age and gender distribution for example, the most common mechanism of injury, how many were transferred from other health facilities etc -->

<!-- Try to write this last part of the paragraph as one coherent sentence instead of this list -->
...
## Training
 ...
 <!-- Try to come up with subheadings that better describe what happened during each step, if you feel that you need subheadings at all. -->

Using the training sample the optimal cutoffs for the logistic regression was found maximizing the youden index. The optimal cutoffs where `r boot.cis$cut.rr[["pe"]]`, `boot.cis$cut.rr[["pe"]]` and 14 for SPB, RR and GCS respectively. and the corresponding confidence intervals can be seen in table 2 <!-- This last part is not a complete sentence. Also make sure that you extract data on the cutoffs on all three variables, not only RR -->



## Validation

Using the validation sample, new coefficients and odds ratios (OR) were estimated for the updated model using the cutoffs given in the training sample. The odds ratios can be seen in table 2. 
...
| GCS       | 4.31(9.96-4.69) | `r ors[["new.gcs_score"]]` | 14                      |
Table: Table 2 Original and uppdated OR for the logistic regression model 

<!-- Okey, im a bit lost about this with the optimal cutoffs... as is now its calculated every time you run the study and the new one is used and i thought that you then need to present the spread but isnt it better to first do this and then do the following parts with the same cutoff otherwise the coefficents and ORs could be for different cutoffs and that seems nonsencical? -->

<!-- Well, I suggest that you present the cutoffs with uncertainty (i.e. 95% CI derived from the bootstrap samples), that way you indicate that they differ across bootstraps -->

<!-- Also, should we include the youden index? or some kind of AUC plot for the cutoffs somehow? Or is it enough to say that we maximized it and show the results? -->
...
Using the test sample to calculate real probabilities of ICU admission and comparing to the estimated ones for the original and updated model. Figure 2? shows a comparison between the estimated and observed probabilities and table 4 compares the ICI original and updated model.
...
| NEW seperate | `r CIs$ICI.new`     | `r CIs$diff.ici.qsofa`     | `r CIs$auc.new` |          |
...
Svaghet:
 * Mycket missing data och oklart om det är MCAR eller MAR eller MNAR och så oklart hur mycket som är felrapporterat.
 * ett score av 0 ger fortfarande 30 % risk för intensivvård vilket gör att det är ifrågasättart för direkt användning i klinisk vardag.
 * Bara ett dataset även fast vi använde bootstrap så är det oklart hur generaliserbart det är
 * AUCn är rätt låg
 * TITCO är bara från urbana sjukhus och återspeglar inte landsbyggssjukvård som är övervägande i bl.a. indien.
 * In TITCO most patients are transfered till medverkande sjukhusen vilket kan motsvara en viss försening av adekvat vård som kan påverka utfallet. alternativt att dom sjukaste dör innan ankomst eller på annan vårdinrättning. 
 
 Styrka:
 * Väldigt låg ICI och nya modellen stämmer klart bättre än orginalmodellen.
 * Bra att visa att orginal qSOFA inte fungerar rakt av i detta sammanhang oavsett anledning.
 * Trots missing data ett stort data-set
 * Uppenbart kunskapslucka som detta hjälper till att fylla även om inte perfekt.
