...
We identified the following revised optimal cutoffs: `r boot.cis$cut.sbp[["pe"]]` for SBP, `r boot.cis$cut.rr[["pe"]]` for RR and `r boot.cis$cut.gcs[["pe"]]` for GCS. The corresponding confidence intervals and updated odds ratios can be seen in table 2 together with the original OR.


|           | Original OR     | Uppdated OR(95%CI)         | Optimal cutoffs (95%CI) |
|-----------|-----------------|----------------------------|-------------------------|
...
The estimated probability of ICU admission and corresponding OR given the OR from table 2 and the distribution of the individual parameters in the study are presented in table 3 and 4.  
...
Figure 2? shows a comparison between the estimated and observed probabilities for the new and updated model and table 5 compares the perforamce for the original and updated model.
...
<!-- aim and how it went, --->
The aim of this study was to asses the validity of the qSOFA score in predicting ICU admissions in trauma patients. In the context of urban Indian hospitals the original qSOFA score with its original cutoffs and coefficients did not accurately predict ICU admission. The model underestimated the probability of ICU admission for scores of 0-2 and overestimated the probability for patients with a score of 3 as can be seen in _FIGURE showing ICI plots_. When updating the model adjusting the cutoffs and coefficients the new model performs significantly better.


<!-- usage and implication -->
The estimated probabilities for the new model ranges from `r boot.cis$est.prob.sum.new.none[["pe"]]` to `r boot.cis$est.prob.sum.new.three[["pe"]]` where the highest value, corresponding to a score of three, can be said to be a clear overestimation and not accounting for a large portion of patients. A more realistic span is that off 0-2 points, or a probability of `r boot.cis$est.prob.sum.new.none[["pe"]]` to `r boot.cis$est.prob.sum.new.two[["pe"]]`. The updated model can not in itself be used to exclude patients with a low score from the need for intensive care nor can it with certainty predict that patients with a high score will be admitted to the ICU. Contrast this to the original use of the qSOFA score where a score of 0 correspond to a mortality of 2-3  <!-- approx taken from figure --> percent and a score of 2-3 correspond to a mortality of 7-20%<!-- taken from figure ish-->. This makes the original qSOFA in the context of screening for severe infections a good tool to sort patients with a low score to also be of a low risk of dying. 
