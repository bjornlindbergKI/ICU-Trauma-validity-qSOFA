...
Results
========
The TITCO cohort included `r results$n.cohort` patients, from which `r results$n.younger.than.18` where excluded for being under the age of 18 leaving `r results$n.adults` patients. Next `r results$n.incl2 ` patients died between arrival and admission and where thus excluded from the sample leaving `r results$n.included` patients. Furthermore `r results$n.NA_TOT` patients where excluded due to missing and non recorded data in either admission to the ICU (`r results$n.NA_ICU`), Systolic blood pressure  (`r results$n.NA_SBP`), Respiratory rate (`r results$n.NA_RR`), GCS: (`r results$n.NA_GCS`). Excluding data points with missing data leaves `r results$n.complete` data points to be included in the final study sample. 

The median age of the final study sample was `r tabOne$ContTable$Overall["Age","mean"]` with predominantly male participants, `r tabOne$CatTable$Overall$Sex["2","percent"]`%. The most common type of injury was road traffic injuries with ```r tabOne$CatTable$Overall$`Mode of injury`["6","percent"]```% followed by fall injuries and assaults accouting for ```r tabOne$CatTable$Overall$`Mode of injury`["3","percent"] ```% and  ```r tabOne$CatTable$Overall$`Mode of injury`["1","percent"] ```% respectively. Furthermore `r  tabOne$CatTable$Overall$Transported["2","percent"]`% of patients were transported to a participating hospital from another health care facility. Overall in the study population ```r tabOne$CatTable$Overall$`Admitted to the ICU`["2","percent"] ```% of patients had to be treated for some time in the ICU. The data is presented in \@ref(tab:table1) with corresponding standard deviations. 
...
Using the training sample the optimal cutoffs of the logistic regressions for each parameter was found maximizing the youden index. The optimal cutoffs where calculated to be `r boot.cis$cut.sbp[["pe"]]`, `r boot.cis$cut.rr[["pe"]]` and `r boot.cis$cut.gcs[["pe"]]` for SPB, RR and GCS respectively and the corresponding confidence intervals can be seen in table 2.
...
Table: Table 4. Comparission of performence,ICI and AUC, between the new and original model as well as the ICI for the combined score for the original and updated.

Discussion
========

<!-- aim and how it went, --->
The aim of this study was to asses the validity of the qSOFA score in predicting ICU admissions in trauma patients. In the context of urban Indian hospitals the original qSOFA score with its original cutoffs and coefficients did not accurately predict the rate of ICU admissions. The model underestimated the rate for scores of 0-2 and overestimated the rate for patients with a score of 3 as can be seen in _FIGURE showing ICI plots_. When updating the model adjusting the cutoffs and coefficients the new model performs significantly better and as well as to be expected using a single database. 

<!-- usage and implication -->
The estimated probabilities ranges from `r boot.cis$est.prob.sum.new.none[["pe"]]` to `r boot.cis$est.prob.sum.new.three[["pe"]]`. 
Where the highest value can be said to be a clear overestimation not accounting for a large portion of patient. A more realistic span is that off 0-2 points, or a probability of `r boot.cis$est.prob.sum.new.none[["pe"]]` to `r boot.cis$est.prob.sum.new.two[["pe"]]`. Because of this narrow span and high baseline risk the model would not be of clinical use as a stand alone metric deciding or predicting ICU admissions but rather as a tool indicating which patients are slightly more or less likely to need it.  

It can also be noted that neither the Odds Ratios for SBP nor RR is a significant result correlated to a higher risk of ICU admisson and the only significant parameter in this case is that of GCS. However this can be partially explained by the fact that a new cutoff where calculated for each bootstrap resulting in different cutoffs used and therefore resulting in an increased uncertainty. The data seem to suggest a correlation between ICU admission and RR and SBP and therefore use it in a scoring model, although this study could not give a statistically significant result the those regards. It could also be that the different parameters interacted with each other either enhancing or decreasing the effect and risk of ICU admission when present at the same time wich was not taken into consideration in this model where the different parameters where presumed to be independent of each other.   

<!--limitations --->
Because of the composition of the TITCO database the results are likely to be generalizable to other urban hospitals of similar economic and resource setting. It is not clear whether the results can be used in rural settings or setting with higher or lower resource settings. It should also be noted that _percent Transported ish 60_ of the patients where for an unspecified time first treated at another hospital and then later transferred to one of the participating hospitals. Because of this the parameters used in this study only shows the first ones taken at arrival to the participating hospital and not always the first ones taken after the injury and does not take into account any interventions performed before arrival.

Furthermore out of the `r results$n.adults` patients above the age of 18 only `r results$n.complete` had complete data, or `r results$n.complete/results$n.adults`%. With this amount of missing data it is unclear if the data can be said to be missing completely at random or if there are any underlying factors contributing to the missing of data that could skew the results leading to a higher uncertainty regardless of bootstraping.
