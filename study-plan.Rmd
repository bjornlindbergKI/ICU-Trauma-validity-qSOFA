...
<!-- Revise the rest of this paragraph based on my first couple of examples --> <!-- i did not understand this syntax so I'm gonna have to take this another day... -->
The median age was `r tabOne$ContTable$Overall["Age","mean"]` with predominantly `r tolower(most.common.sex)` participants (`r p.sex`%). The most common type of injury was `r tolower(most.common.mechanism)`s with `r p.mechanism`% followed by `r tolower(second.most.common.mechanism)`s and `r tolower(third.most.common.mechanism)` accouting for ```r tabOne$CatTable$Overall$`Mode of injury`["3","percent"] ```% and  ```r tabOne$CatTable$Overall$`Mode of injury`["1","percent"] ```% respectively. Furthermore `r  tabOne$CatTable$Overall$Transported["2","percent"]`% of patients were transported to a participating hospital from another health care facility. Overall in the study population ```r tabOne$CatTable$Overall$`Admitted to the ICU`["2","percent"] ```% of patients had to be treated for some time in the ICU. The data is presented in \@ref(tab:table1) with corresponding standard deviations. 
...
```
```{r table1, echo = FALSE}
 kableone(CreateTableOne(data = results$data.table1), caption = "Sample characteristics", label = "table1")
```


We identified the following revised optimal cutoffs: `r boot.cis$cut.sbp[["pe"]]` for SBP, `r boot.cis$cut.rr[["pe"]]` for RR and `r boot.cis$cut.gcs[["pe"]]` for GCS and the corresponding confidence intervals can be seen in table 2.

<!-- Okay, so you're repeating quite a bit of the methods in the results. I don't think that's necessary. Instead I suggest you focus on presenting the results (i.e. the numbers that you find) and introducing/summarizing tables and figures --> <!-- Well yes i can see that havnt had the time to rewrite it this week but ill fix it. -->

The updated odds ratios can  can be seen in table 2. 

Furthermore the new coefficents and the original coefficents were used to create odds ratios for the original and uppdated qSOFA scores using the distribution in the validation sample and yielded the results given in table 3 and 4 
...
The test sample was used to calculate real probabilities of ICU admission and comparing to the estimated probabilities from the original and updated model. Figure 2? shows a comparison between the estimated and observed probabilities and table 4 compares the ICI original and updated model.
...
Table: Table 4. Comparision of performance, ICI and AUC, between the new and original model as well as the ICI for the combined score for the original and updated.
