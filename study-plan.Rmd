...
<!-- Revise the rest of this paragraph based on my first couple of examples -->
...
Using the training sample the optimal cutoffs of the logistic regressions for each parameter was found maximizing the youden index. The optimal cutoffs where calculated to be `r boot.cis$cut.sbp[["pe"]]`, `r boot.cis$cut.rr[["pe"]]` and `r boot.cis$cut.gcs[["pe"]]` for SPB, RR and GCS respectively and the corresponding confidence intervals can be seen in table 2. <!-- This paragraph is unnecessarily long. The first sentence can be removed. The second can be revised to start with: We identified the following revised optimal cutoffs:... or something similar --> 

<!-- Okay, so you're repeating quite a bit of the methods in the results. I don't think that's necessary. Instead I suggest you focus on presenting the results (i.e. the numbers that you find) and introducing/summarizing tables and figures -->
Then using the validation sample, new coefficients and odds ratios (OR) were estimated for the updated model using the cutoffs given in the training sample from each bootstrap. The odds ratios can be seen in table 2. 
...
<!-- aim and how it went, --->
The aim of this study was to asses the validity of the qSOFA score in predicting ICU admissions in trauma patients. In the context of urban Indian hospitals the original qSOFA score with its original cutoffs and coefficients did not accurately predict ICU admission. The model underestimated the rate<!-- Why do you focus on the rate here? The rate is not included in the aim or methods, so I suggest that you stick to just "ICU admission" or "probability of ICU admission" -->for scores of 0-2 and overestimated the rate for patients with a score of 3 as can be seen in _FIGURE showing ICI plots_. When updating the model adjusting the cutoffs and coefficients the new model performs significantly better and as well as to be expected using a single database<!-- The meaning of this last part of the sentence (start with "as well as to") is unlclear to me. I suggest you remove or revise to clarify-->. 

<!-- usage and implication -->
The estimated probabilities <!-- Note that you use probability here rather than rate-->ranges from `r boot.cis$est.prob.sum.new.none[["pe"]]` to `r boot.cis$est.prob.sum.new.three[["pe"]]`. 
Where the highest value can be said to be a clear overestimation not accounting for a large portion of patients<!-- Clarify what model you are referring to here-->. A more realistic span is that off 0-2 points, or a probability of `r boot.cis$est.prob.sum.new.none[["pe"]]` to `r boot.cis$est.prob.sum.new.two[["pe"]]`. Because of this narrow span and high baseline risk the model would not be of clinical use as a stand alone metric deciding or predicting ICU admissions but rather as a tool indicating which patients are slightly more or less likely to need it<!-- This last sentence is a bit long. Try to divide into two or three to clarify what you mean-->.

It can also be noted that neither SBP nor RR is significantly associated with a higher risk of ICU admisson. Only GCS is significantly associated with ICU admission. This can be partially explained by the fact that a new cutoff where calculated for each bootstrap resulting in different cutoffs used and therefore resulting in an increased uncertainty. The data seem to suggest a correlation between ICU admission and RR and SBP and therefore use it in a scoring model, although this study could not give a statistically significant result the those regards. It could also be that the different parameters interacted with each other either enhancing or decreasing the effect and risk of ICU admission when present at the same time wich was not taken into consideration in this model where the different parameters where presumed to be independent of each other.<!-- You tend to write quite long sentences with several key points. Try to look at each sentence and make sure you only have one key point per sentence. Also, try to cut away extra words. "The fact that" can almost always be replace by just "that" and "seems to <verb>" can be just <verb>-->

<!--limitations --->
Because of the composition of the TITCO database the results are likely to be generalizable to other urban hospitals of similar economic and resource setting<!-- Why do you think that?-->. It is not clear whether the results can be used in rural settings or setting with higher or lower resource settings. It should also be noted that _percent Transported ish 60_ of the patients where for an unspecified time first treated at another hospital and then later transferred to one of the participating hospitals. Because of this the parameters used in this study only shows the first ones taken at arrival to the participating hospital and not always the first ones taken after the injury and does not take into account any interventions performed before arrival<!-- Or between arrival and admission to the ICU-->.

Furthermore out of the `r results$n.adults` patients above the age of 18 only `r results$n.complete` had complete data, or `r results$n.complete/results$n.adults`%. With this amount of missing data it is unclear if the data can be said to be missing completely at random or if there are any underlying factors contributing to the missing of data that could skew the results leading to a higher uncertainty regardless of bootstraping<!-- Does the amount of missing data really say anything about the pattern of missing data?-->.
